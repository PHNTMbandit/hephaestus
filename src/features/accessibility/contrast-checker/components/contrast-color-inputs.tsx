...
export const ContrastColorInputs = ({
  ...
  const handleForegroundValueChange = (color: string) => {
    dispatch({ type: 'SET_FOREGROUND_COLOR', payload: { color: color } })
  }

  const handleBackgroundValueChange = (color: string) => {
    dispatch({ type: 'SET_BACKGROUND_COLOR', payload: { color: color } })
  }
